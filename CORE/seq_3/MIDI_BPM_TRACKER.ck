public class MIDI_BPM_TRACKER {

  string device;
  int midi_clock_interval_update;
  int bpm_interval_update;
    ...
    while(1) {
      ...
      if (started) {
        if (first_sync && total_midi_clocks > 0 ) { // At least one midi clock received
          ...
          last_midi_clock_time - total_midi_clocks * data.tick / 24 => spp_ref_time;
          // Adjust SPP with midi beats inside message
          spp_ref_time - midi_beats * data.tick / 4 => spp_ref_time;
          ...
          if (total_midi_clocks > last_total_midi_clocks + midi_clock_interval_update) { // At least one midi clock received
            ...
            ////////////////////////////////////////////////
            // BPM
            if (bpm_track && total_midi_clocks > bpm_last_total_midi_clocks + bpm_interval_update) {

              (total_midi_clocks - bpm_last_total_midi_clocks ) * 60::second / ( ( last_midi_clock_time - bpm_last_midi_clock_time ) * 24 ) => bpm;
  ...
  fun void go(string dev,  int mciu, int bpmiu, dur off, int bpmt){
    dev => device;
    mciu => midi_clock_interval_update;
    bpmiu => bpm_interval_update;
