...
t.scale.size(0);
t.scale << 1 << 3 << 1 << 2 << 3 << 2;
t.reg(SERUM0 s0); s0.config(6, 0);  //data.tick * 8 => t.max; //60::ms => t.glide;  // t.lyd(); // t.ion(); // t.mix();// t.dor();// t.aeo(); // t.phr();// t.loc();
...
3 => gsin.op;
STEPC stepc2; stepc2.init(HW.lpd8.potar[1][3], 0 /* min */, 40 /* max */, 50::ms /* transition_dur */);
stepc2.out => gsin;  
