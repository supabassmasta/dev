...
STDUCKMASTER duckm;
duckm.connect(last $ ST, 9. /* In Gain */, .04 /* Tresh */, .2 /* Slope */, 6::ms /* Attack */, 16::ms /* Release */ );      duckm $ ST @=>  last; 

  1::samp => now; // let seq() be sporked to compute length
  t.s.duration - 1::samp => now;
}
//spork ~ KICK("*4 k___ k___ k___ k___",0,1.);
fun void KICK2(string seq, int tomix, float g) {
  local_delay => now;
...
fun void BASS0 (string seq, int tomix, float g) {
  ...
  stsynclpfx0.freq(18 * 10 /* Base */, 25 * 10 /* Variable */, 1.0 /* Q */);
...
fun void SYNTNOISE (string seq, int n, float lpf_f, dur gldur, dur d, int tomix, float v) {
  local_delay => now;

  TONE t;
  t.reg(NOISE0 s0);  //data.tick * 8 => t.max; 
  gldur => t.glide;  // t.lyd(); // t.ion(); // t.mix();//
  //t.set_scale(data.scale.my_string);// t.aeo(); // t.phr();// t.loc(); t.double_harmonic();
  t.set_scale(data.scale.my_string);
  // _ = pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = shift base note , ! = force new note , # = sharp , ^ = bemol  
...
STAUTOPAN autopan;
autopan.connect(last $ ST, .9 /* span 0..1 */, data.tick * 2 / 1 /* period */, 0.5 /* phase 0..1 */ );       autopan $ ST @=>  last; 
...
//  spork ~ SYNTNOISE("*4 5231__" /* seq */, 2 /* Serum00 synt */, 9 * 100 /* lpf_f */, 5::ms /* glide dur */,16*data.tick,0,.25);
...
fun void  BEAT0x8  (int n, int add_a_last_Kick, int remove_last_beats){ 
  ...
  for (0 => int i; i <  n     ; i++) {
   ...
   spork ~  BASS0_ATTACK ("*4     aaaa aaaa aaaa aaaa aaaa aaaa aaaa aaaa  ", 0.5 /* rate */,0, .11 /* g */); 
   8 * data.tick => w.wait;
  }
  if ( remove_last_beats  ){
    string seq;
   ...
   spork ~  BASS0_ATTACK ("*4  " + bassatk, 0.5 /* rate */,0, .10 /* g */); 
  ...
  while(now <end) {
     ...
     if(maybe) {
       if(maybe) {
         ...
         s0.set_chunk(12); 182 => sin0.freq; 162 *10.0 => sin0.gain; 2.1 => s0.gain;
         data.tick * 1./1. => now;
       }
       }  else {
         if(maybe) {
         ...
         s0.set_chunk(15); 440=> sin0.freq; 205 *10.0 => sin0.gain; 2.1 => s0.gain;
         data.tick * 1./2. => now;
         }
       } 
...
fun void EFFECT2   (){ 
  ...
  STFILTERX stlpfx0; LPF_XFACTORY stlpfx0_fact;
  stlpfx0.connect(last $ ST ,  stlpfx0_fact, crossover /* freq */ , 1.0 /* Q */ , 1 /* order */, 1 /* channels */ );       stlpfx0 $ ST @=>  last;  
...
sthpfx0.connect(stmix $ ST ,  sthpfx0_fact, crossover /* freq */ , 1.0 /* Q */ , 1 /* order */, 1 /* channels */ );       sthpfx0 $ ST @=>  last;  
...
STAUTOPAN autopan;
autopan.connect(last $ ST, .3 /* span 0..1 */, data.tick * 5 / 1 /* period */, 0.75 /* phase 0..1 */ );       autopan $ ST @=>  last; 
...
STAUTOPAN autopan;
autopan.connect(last $ ST, .3 /* span 0..1 */, data.tick * 5 / 1 /* period */, 0.25 /* phase 0..1 */ );       autopan $ ST @=>  last; 
...
///////////////////////////////////////////////////////////////////////////////////
   ...
   fun void  AMB (){ 
     ...
     spork ~ SPECTR (31/*note*/,19/*file*/,0.3/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 2, 0.7); 
     2 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28/*note*/,19/*file*/,0.3/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 2, 0.7); 
     4 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28/*note*/,22/*file*/,0.4/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.3/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 2 * 8 * data.tick, 2, 1.2); 
     2 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28/*note*/,20/*file*/,0.4/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,1/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 2 * 8 * data.tick, 2, 0.6); 
     3 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28 + 24/*note*/,7/*file*/,0.4/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 2, 0.5); 
     3 * 8 * data.tick => w.wait;
     spork ~ SPECTR (31/*note*/,19/*file*/,0.3/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 2, 0.7); 
     2 * 8 * data.tick => w.wait;
   } 
   ...
   fun void  AMB1 (){ 
     ...
     spork ~ SPECTR (31/*note*/,19/*file*/,0.3/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 5, 0.7); 
     2 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28/*note*/,19/*file*/,0.3/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 6, 0.7); 
     1 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28/*note*/,20/*file*/,0.4/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,1/*whisperize*/,0.0/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 5 * 8 * data.tick, 5, 0.6); 
     1 * 8 * data.tick => w.wait;
     spork ~ SPECTR (28/*note*/,22/*file*/,0.4/*loopStart*/,0.9/*loopEnd*/,0./*semiToneShift*/,0/*robotize*/,0/*whisperize*/,0.3/*spectralBlur*/,0.0/*spectralGate*/,3 * 8 * data.tick/*att*/,6 * 8 * data.tick/*rel*/, 4 * 8 * data.tick, 6, 1.2); 
...
fun void  CRAZ  (){ 
  for (0 => int i; i < 8      ; i++) {
    spork ~   CRAZYMOD (" *4  " + RAND.seq("!1_8_,!f!f!1_,!1!8!8_,!1!1!1_,!8!8__,!8!5!1_,!FF__,!F!B!__", 2 + i), 20/*n*/,200*100/*cut*/,1,0.5); 
    spork ~   CRAZYMOD2 (" *4 " + RAND.seq("____,___8,___1,____,___F,____,___f", 2 + i), 20/*n*/,200*100/*cut*/,3,0.5); 
    2 * 8 * data.tick => w.wait;
  }
} 
fun void  CRAZ2  (){ 
  for (0 => int i; i < 8      ; i++) {
    ...
    spork ~   CRAZYMOD2 (" *4 " + RAND.seq("____,___8,___1,____,___F,____,___f", 2 + i), 20/*n*/,200*100/*cut*/,3,0.5); 
    1 * 8 * data.tick => w.wait;
    spork ~ SYNTNOISE("}c *4 " + RAND.seq("_,8/1, F//1, B/5,5/F",  12 ) /* seq */, 2 /* Serum00 synt */, 9 * 100 /* lpf_f */, 5::ms /* glide dur */,(1 + i)*data.tick,3,0.7);
    1 * 8 * data.tick => w.wait;
  }
} 
...
//  spork ~ SYNTGLIDE("}c *4 231" /* seq */, 2 /* Serum00 synt */, 9 * 100 /* lpf_f */, 5::ms /* glide dur */,16*data.tick,11,.25);
//    spork ~ ERAMPLPF (11/*mixin*/,2*8*data.tick,":4 1//33//8"/*gseq*/,":4 M////z"/*lpfseq*/,1/*lpforder*/,1,2.3);
//  spork ~ SYNTGLIDE("}c *4 123" /* seq */, 3 /* Serum00 synt */, 9 * 100 /* lpf_f */, 5::ms /* glide dur */,16*data.tick,11,.25);
//    spork ~ ERAMPLPF (11/*mixin*/,2*8*data.tick,":4 1//33//8"/*gseq*/,":4 M////z"/*lpfseq*/,1/*lpforder*/,1,2.3);
...
//  spork ~ SYNTGLIDE("}c *4 2314538231453B" /* seq */, 2 /* Serum00 synt */, 9 * 100 /* lpf_f */, 5::ms /* glide dur */,32*data.tick,11,.25);
// spork ~ BEAT0x8  (4, 1/*add_a_last_Kick*/, 4 /*remove_last_beats*/);
//   4 * 8 * data.tick => w.wait;
