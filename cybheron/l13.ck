...
fun void BASS0 (string seq, int tomix, float g) {
  ...
  STADSR stadsr;
  stadsr.set(.0::ms /* Attack */, 6::ms /* Decay */, 1. /* Sustain */, -0.45/* Sustain dur of Relative release pos (float) */,  30::ms /* release */);
...
//  spork ~   CRAZYMOD (" *4 " + RAND.seq("!1_8_,!f!f!1_,!1!8!8_,!1!1!1_,!8!8__,!8!5!1_,!FF__,!F!B!__", 12), 20/*n*/,200*100/*cut*/,0,0.5); 
...
//    spork ~   CRAZYMOD2 (" *4 " + RAND.seq("____,___8,___1,____,___F,____,___f", 24), 20/*n*/,200*100/*cut*/,0,0.5); 
...
//    spork ~   CRAZYMOD2 (" *4 " + RAND.seq("____,___8,___1,____,___F,____,___f", 24), 20/*n*/,200*100/*cut*/,8*data.tick/*d*/,0,0.5); 
...
STSYNCFILTERX stsynclpfx0; LPF_XFACTORY stsynclpfx0_fact;
stsynclpfx0.freq(50 * 10 /* Base */, 76 * 100 /* Variable */, 1.5 /* Q */);
stsynclpfx0.adsr_set(.1 /* Relative Attack */, .5/* Relative Decay */, 0.0001 /* Sustain */, .1 /* Relative Sustain dur */, 0.1 /* Relative release */);
...
//STFILTERX stlpfx0; LPF_XFACTORY stlpfx0_fact;
//stlpfx0.connect(last $ ST ,  stlpfx0_fact, 11* 100.0 /* freq */ , 4.0 /* Q */ , 2 /* order */, 1 /* channels */ );       stlpfx0 $ ST @=>  last;  

//STAUTOFILTERX stautoresx0; RES_XFACTORY stautoresx0_fact;
//stautoresx0.connect(last $ ST ,  stautoresx0_fact, 0.6 /* Q */, 1 * 100 /* freq base */, 8 * 100 /* freq var */, data.tick * 16 / 2 /* modulation period */, 1 /* order */, 1 /* channels */ , 1::ms /* update period */ );       stautoresx0 $ ST @=>  last;  
...
//while(1) {
//       <<<"stsynclpfx0.nio.q_padsr : ",stsynclpfx0.nio.q_padsr.last()>>>;
//
//       300::ms => now;
//}
// 
...
// SUPERGLIDES Mod
TriOsc tri0 => OFFSET ofs_superglides_mod;
...
TriOsc tri1 =>  ofs_superglides_mod;
0.55 => tri1.freq;
150.0 => tri1.gain;
0.2 => tri1.width;
...
class syntMultiSERUM00 extends SYNT{
...
inlet => detune[i] => s[i] => final; w => s[i].width;   1. => detune[i].gain;    .4 => s[i].gain; i++;  
inlet => detune[i] => s[i] => final; w => s[i].width;   1.01 => detune[i].gain;    .4 => s[i].gain; i++;  
inlet => detune[i] => s[i] => final; w => s[i].width;   0.993 => detune[i].gain;    .4 => s[i].gain; i++;  

        fun void on()  { }  fun void off() { }  fun void new_note(int idx)  { } 0 => own_adsr;
} 

fun void  SUPERGLIDES2  (string cutseq,  dur d, int tomix, float g){ 

  ST stmonoin; stmonoin $ ST @=> ST @ last;
  ofs_superglides_mod => Delay de => syntMultiSERUM00 ss  => stmonoin.mono_in ; 
  1*data.tick => de.max;
  1*data.tick => de.delay;
...
//  STFILTERX stlpfx0; LPF_XFACTORY stlpfx0_fact;
//  stlpfx0.connect(last $ ST ,  stlpfx0_fact, 32* 100.0 /* freq */ , 1.0 /* Q */ , 3 /* order */, 1 /* channels */ );       stlpfx0 $ ST @=>  last;  

//STAUTOFILTERX stautolpfx0; LPF_XFACTORY stautolpfx0_fact;
//stautolpfx0.connect(last $ ST ,  stautolpfx0_fact, 4.0 /* Q */, 15 * 100 /* freq base */, 12 * 100 /* freq var */, data.tick * 17 *2  /* modulation period */, 2 /* order */, 1 /* channels */ , 1::ms /* update period */ );       stautolpfx0 $ ST @=>  last;  


  STCONVREV stconvrev;
  stconvrev.connect(last $ ST , 8/* ir index */, 1 /* chans */, 10::ms /* pre delay*/, .01 * 5 /* rev gain */  , 0.9 /* dry gain */  );       stconvrev $ ST @=>  last;  
  while(1) {
         100::ms => now;
  }
...
stautobpfx0.connect(last $ ST ,  stautobpfx0_fact, 1.0 /* Q */, 10 * 100 /* freq base */, 30 * 100 /* freq var */, data.tick * 34  /* modulation period */, 1 /* order */, 1 /* channels */ , 1::ms /* update period */ );       stautobpfx0 $ ST @=>  last;  
3 => stautobpfx0.gain;
  STMIX stmix2;
  stmix2.send(last, mixer + 1);
  //stmix.receive(11); stmix $ ST @=> ST @ last; 
...
//  stconvrev.connect(last $ ST , 17/* ir index */, 1 /* chans */, 10::ms /* pre delay*/, .16 /* rev gain */  , 0.9 /* dry gain */  );       stconvrev $ ST @=>  last;  
  STCONVREV stconvrev;
  stconvrev.connect(last $ ST , 25/* ir index */, 1 /* chans */, 30::ms /* pre delay*/, .03 /* rev gain */  , 1. /* dry gain */  );       stconvrev $ ST @=>  last;  
...
fun void EFFECT5   (){ 
  ...
  STAUTOPAN autopan;
  autopan.connect(last $ ST, .9 /* span 0..1 */, data.tick * 8 / 3 /* period */, 0.95 /* phase 0..1 */ );       autopan $ ST @=>  last; 
  STMIX stmix2;
  stmix2.send(last, mixer + 7);
  while(1) {
         100::ms => now;
  }
...
fun void EFFECT6   (){ 
  ...
  STAUTOPAN autopan;
  autopan.connect(last $ ST, .9 /* span 0..1 */, data.tick * 8 / 3 /* period */, 0.5 /* phase 0..1 */ );       autopan $ ST @=>  last; 
...
fun void EFFECT7   (){ 
  STMIX stmix;
  stmix.receive(mixer + 7); stmix $ ST @=> ST @ last; 
...
fun void EFFECT8   (){ 
  STMIX stmix;
  stmix.receive(mixer + 8); stmix $ ST @=> ST @ last; 

STECHO ech;
ech.connect(last $ ST , data.tick * 3 / 4 , .7);  ech $ ST @=>  last; 

  STMIX stmix2;
  stmix2.send(last, mixer + 4);
  while(1) {
         100::ms => now;
  }
...
fun void BEAT1  (int n){ 
  for (0 => int i; i <  n     ; i++) {
    spork ~KICK("*4 k___ k___ k___ k___k___ k___ k___ k___",0,1.);
    spork ~ BASS0("*2  _3 _1 _3 _1 _3 _1 _3 _1  ",0,1.);
...
fun void BEAT2_8x8  (){ 
    ...
    8 * data.tick => w.wait;
    spork ~ KICK("*4     k___ k___ k___ k___k___ k___ k___ k___",0,1.);
    spork ~ BASS0("*2         _3 _1 _3 _1 _3 _1 _3 _8  ",0,1.);
    spork ~ BASS0_ATTACK ("*2 _1 _1 _1 _1  _1 _1 _1 _1  ", 0.7 /* rate */,0, .16 /* g */); 
    8 * data.tick => w.wait;
    spork ~ KICK("*4     k___ k___ k___ k___k___ k___ k___ k___",0,1.);
    spork ~ BASS0("*2         _3 _1 _3 _1 _3 _1 _3 _1  ",0,1.);
    ...
    spork ~ BASS0("*2         _3 _1 _3 _1 _3 _1 _3 !3!3  ",0,1.);
    spork ~ BASS0_ATTACK ("*2 _1 _1 _1 _1  _1 _1 _1 11  ", 0.7 /* rate */,0, .16 /* g */); 
    8 * data.tick => w.wait;
    ...
    spork ~ BASS0_ATTACK ("*2 _1 _1 _1 _1  _1 _1 _1 _1  ", 0.7 /* rate */,0, .16 /* g */); 
    8 * data.tick => w.wait;
    spork ~ KICK("*4     k___ k___ k___ k___k___ k___ ____ k_k_",0,1.);
    spork ~ BASS0("*2         _3 _1 _3 _1 _3 _1 !2!3 __  ",0,1.);
    spork ~ BASS0_ATTACK ("*2 _1 _1 _1 _1  _1 _1 _1 _1  ", 0.7 /* rate */,0, .16 /* g */); 
    8 * data.tick => w.wait;
    spork ~ KICK("*4     k___ k___ k___ k___k___ k___ k___ k___",0,1.);
    spork ~ BASS0("*2         _3 _1 _3 _1 _3 _1 _3 _1  ",0,1.);
    spork ~ BASS0_ATTACK ("*2 _1 _1 _1 _1  _1 _1 _1 _1  ", 0.7 /* rate */,0, .16 /* g */); 
    8 * data.tick => w.wait;
    spork ~ KICK("*4     k___ k___ k___ k___k___ k___ k_k_ __k_",0,1.);
    spork ~ BASS0("*2         _3 _1 _3 _1 _3 _1 __ __  ",0,1.);
    spork ~ BASS0_ATTACK ("*2 _1 _1 _1 _1  _1 _1 __ __  ", 0.7 /* rate */,0, .16 /* g */); 
    8 * data.tick => w.wait;
} 

fun void  HH1_8x8  (){ 
   
  spork ~  TRIBAL_CUSTOM("*4    __h_ __hh h_h_ __hh h_h_ __hh h_h_ __hh ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ ____ __j_ ____ ____ ____ __j_ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _______s _s_____s ______ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  spork ~  TRIBAL_CUSTOM("*4 __h_ __hh h_h_ __hh h_h_ __hh h_h_ __hh ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ __j_ __j_ ____ ____ __j_ __j_ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _s_____s _s_____s _s____ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  spork ~  TRIBAL_CUSTOM("*4 __h_ __hh h_h_ __hh h_h_ __hh h_h_ __hh ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ ____ __j_ ____ ____ ____ __j_ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _______s _s_____s ______ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  spork ~  TRIBAL_CUSTOM("*4 __h_ __hh h_h_ __hh h_h_ __hh h_h_ ____ ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ __j_ __j_ ____ ____ __j_ __j_ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _s_____s _ss____s _s_s__ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  ...
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _______s _s_____s ______ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  spork ~  TRIBAL_CUSTOM("*4 __h_ __hh h_h_ __hh h_h_ __hh ____ rhhh ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ __j_ __j_ ____ ____ __j_ ____ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _s_____s _s_____s _s____ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  spork ~  TRIBAL_CUSTOM("*4 __h_ __hh h_h_ __hh h_h_ __hh h_h_ __hh ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ ____ __j_ ____ ____ ____ __j_ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _______s _s_____s ______ss _s__ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
  spork ~  TRIBAL_CUSTOM("*4 __h_ __hh h_h_ __hh h_h_ __hh hhhh _hhh ",0,1.5); 
  spork ~  TRIBAL_CUSTOM("*4 {6 ____ __j_ __j_ ____ ____ __j_ __j_ ____  ",0,0.5); 
  spork ~  TRIBAL_CUSTOM("*4 }8 ___s _s_____s _sss_s_s _s_s__ss _ss_ ",0,0.3); 
  1 * 8 * data.tick => w.wait;
...
fun void  PLOCLOOP_4x8  (){ 
     spork ~ PLOC("}c}c}c  1_F_", 17/*n*/,75*100/*cut*/,2,1.4); 
1 * 8 * data.tick => w.wait;
  spork ~ PLOC("}c}c}c  F_1_", 17/*n*/,75*100/*cut*/,2,1.4); 
1 * 8 * data.tick => w.wait;
  spork ~ PLOC("}c}c}c  1_F_", 17/*n*/,75*100/*cut*/,2,1.4); 
1 * 8 * data.tick => w.wait;
  spork ~ PLOC("}c}c}c  F*4_3:4 !1_ ", 17/*n*/,75*100/*cut*/,2,1.4); 
1 * 8 * data.tick => w.wait;

} 

fun void ACIDLOOP_4x8 () {
  spork ~   ACID ("*4   1 " + RAND.seq("!3!2!1,]1,[1,[1,!4!3!2!1,!2!1,!3,!2",64) 
           , 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 F" /*target_q*/, (2*8 ) *data.tick , 3, .22);
2 * 8 * data.tick => w.wait;
  spork ~   ACID ("*4 {c 5  " + RAND.seq("!3!2!1,]1,]1,[1,!4!3!2!1,!2!1,!3,!2",64) 
           , 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 F" /*target_q*/,(2*8) *data.tick , 3, .22);
2 * 8 * data.tick => w.wait;
}
...
fun void  BREAK1  (){ 
       while(data.bpm < 180) {
      data.bpm + 3.7 => data.bpm;   (60.0/data.bpm)::second => data.tick;
      <<<"data.bpm", data.bpm>>>;
      spork ~KICK("*4 k___ k___ ",0,1.);
      spork ~ BASS0("*2  _3  _1 ",0,1.);
      spork ~  BASS0_ATTACK ("*2     _1 _1   ", 0.7 /* rate */,0, .16 /* g */); 
      1 * 2 * data.tick => w.wait;
    }
    while(data.bpm < 250) {
      data.bpm + 9.7 => data.bpm;   (60.0/data.bpm)::second => data.tick;
      <<<"data.bpm", data.bpm>>>;
      spork ~KICK("*4 k___ k___ ",0,1.);
      spork ~ BASS0("*2  _3  _1 ",0,1.);
      spork ~  BASS0_ATTACK ("*2     _1 _1   ", 0.7 /* rate */,0, .16 /* g */); 
    ...
    while(data.bpm < 1250) {
      data.bpm + 27.7 => data.bpm;   (60.0/data.bpm)::second => data.tick;
      <<<"data.bpm", data.bpm>>>;
      spork ~KICK("*4 k___ k___ ",0,1.);
      spork ~ BASS0("*2  _3  _1 ",0,1.);
      spork ~  BASS0_ATTACK ("*2     _1 _1   ", 0.7 /* rate */,0, .16 /* g */); 
...
fun void  BREAK2  (){ 
     spork ~ ERAMPLPF (10/*mixin*/,50*data.tick,":8:8 8/f"/*gseq*/,":4 z"/*lpfseq*/,1/*lpforder*/,3,1.0);

     while(data.bpm < 180) {
        data.bpm + 4.7 => data.bpm;   (60.0/data.bpm)::second => data.tick;
        <<<"data.bpm", data.bpm>>>;
        spork ~   ACID ("*4    " + RAND.seq("!3!2!1,]1,[1,!4!3!2!1,!2!1,!3,!2",12) 
           , 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 F" /*target_q*/, (2 ) *data.tick , 10, .22);
        1 * 2 * data.tick => w.wait;
      }

      while(data.bpm < 350) {
        data.bpm + 7.7 => data.bpm;   (60.0/data.bpm)::second => data.tick;
        <<<"data.bpm", data.bpm>>>;
        spork ~   ACID ("*4    " + RAND.seq("!3!2!1,]1,[1,!4!3!2!1,!2!1,!3,!2",12) 
           , 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 F" /*target_q*/, (2 ) *data.tick , 10, .22);
        1 * 2 * data.tick => w.wait;
      ...
      for (0 => int i; i < 10      ; i++) {
        spork ~   ACID ("*4  !3!2!1!4!3!2!1!2!1"           , 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 F" /*target_q*/, (2 ) *data.tick , 10, .22);
        1 * 2 * data.tick => w.wait;
...
//   spork ~   SUPERGLIDES  ("____ ____ 1111 ____" /*cutseq*/, 2*8 * data.tick/*d*/, 5, 1.); 
//   spork ~   SUPERGLIDES2 ("____ ____ ____11"/*cutseq*/,2*8 * data.tick/*d*/, 6, 1.); 
//   spork ~   BEAT1 (8); 
//
//8 * 8 * data.tick => w.wait;


//       spork ~  SLIDENOISE(300 /* fstart */, 3000 /* fstop */, 44* data.tick /* dur */, .8 /* width */,2,.14); 
//       BREAK2();
//       150 => data.bpm;   (60.0/data.bpm)::second => data.tick;
//       1 * 4 * data.tick => w.wait;
...
//      spork ~   MODU (31, "*8   FFFF____ ____  FFFF____ ____ FFFF " , "f", "6", 3 *1000, 1,                1.10); 
//      spork ~   MODU (31, "*8   ________ ____  ________ ____ ____ FFFF" , "f", "6", 3 *1000, 2,            1.10); 
//  
//  spork ~   PLOCLOOP_4x8 (); 
//    spork ~  ACIDLOOP_4x8();
//  2 * 8 * data.tick => w.wait;
//  
//      spork ~   MODU (31, "*8   FFFF____ ____  FFFF____ ____ FFFF ____ ____ 1111" , "f", "6", 3 *1000, 1,  1.10); 
//      spork ~   MODU (31, "*8   ________ ____  ________ ____ ____ FFFF ____ ____ " , "f", "6", 3 *1000, 2, 1.10); 
//  
//  spork ~   PLOCLOOP_4x8 (); 
//    spork ~  ACIDLOOP_4x8();
//  2 * 8 * data.tick => w.wait;
...
//          1_1_ 1_1_ 1_1_ 3_1_ 3_3_ 1_8_ __3_ 3_3_ 
//          1_1_ 1_1_ 1___ 3_2_ 3___ 1_1_ 3_3_ a_a_ 
//          1_1_ 1_1_ 1_1_ 3_1_ 3_3_ 8_1_ __3_ 8_1_ 
//
//
//          ", 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 F" /*target_q*/, 8*8*data.tick , 1, .60);
//   spork ~   ACID ("*8 {c {c  
//           ____ ____ ____ ____ ____ ____ ____ 1_1_ 
//           ____ ____ ____ ____ ____ ____ __3_ ____ 
//           ____ ____ ____ ____ ____ ____ __3_ 1___ 
//           ____ ____ ____ ____ ____ __1_ ____ __1_ 
// 
//           ", 1231, ":2 8///ff/8" /*target_f*/, ":2 1//88///1" /*base_f*/, ":8:2 1//1" /*target_q*/, 8*8*data.tick , 2, .5);

//   spork ~ BASS0HF("*4  __33 __11 __33 __11 __33 __11 __33 __11    ",0,1.);
//   spork ~ BASS0(" *4   __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1   ",0,1.);
...
/// PLAY OR REC /////////////////
RECTRACK rectrack; "l13.wav"=>rectrack.name_main; 0=>rectrack.compute_mode; 1=>rectrack.rec_mode;8*data.tick=>rectrack.main_extra_time;8*data.tick=>rectrack.end_loop_extra_time;
...
if (rectrack.play_or_rec() ) {
  ...
  //  !!!!!!  Put main code here  !!!!!
   ...
   2 * 8 * data.tick => w.wait;
   spork ~   SUPERGLIDES  ("____ ____ 1111 ____" /*cutseq*/, 2*8 * data.tick/*d*/, 5, 1.); 
   spork ~   SUPERGLIDES2 ("____ ____ ____11__"/*cutseq*/,2*8 * data.tick/*d*/, 6, 1.); 
   2 * 8 * data.tick => w.wait;
   ...
   spork ~  SLIDENOISE(200 /* fstart */, 4000 /* fstop */, 44* data.tick /* dur */, .8 /* width */,2,.14); 
   BREAK1();

   spork ~   SUPERGLIDES (RAND.seq("_1,_1,__11,___,1,_1,__11,__,1111,___,___,___,___,___,___",16)/*cutseq*/, 8*8 * data.tick/*d*/, 5, 1.); 
   spork ~   SUPERGLIDES2 (RAND.seq("_1,_1,__11,___,1,_1,__11,__,1111,___,___,___,___,___,___",16)/*cutseq*/,8*8 * data.tick/*d*/, 6, 1.); 
   ...
   spork ~   BEAT2_8x8 ();
   spork ~   HH1_8x8(); 
   spork ~   TRANCEHHx8 (4, 0); 
   spork ~   SUPERGLIDES (RAND.seq("_1,_1,__11,___,1,_1,__11,__,1111,___,___",16)/*cutseq*/, 8*8 * data.tick/*d*/, 5, 1.); 
   spork ~   SUPERGLIDES2 (RAND.seq("_1,_1,__11,___,1,_1,__11,__,1111,___,___",16)/*cutseq*/,8*8 * data.tick/*d*/, 6, 1.); 
   spork ~   MODU (31, "*8   FFFF____ ____  FFFF____ ____ FFFF " , "f", "6", 3 *1000, 4,                1.10); 
   spork ~   MODU (31, "*8   ________ ____  ________ ____ ____ FFFF" , "f", "6", 3 *1000, 8,            1.10); 
   spork ~   PLOCLOOP_4x8 (); 
   spork ~  ACIDLOOP_4x8();
...
for (0 => int i; i <  1      ; i++) {
   ...
   spork ~   SUPERGLIDES (RAND.seq("_1,_1,__11,___,1,_1,__11,__,1111",16)/*cutseq*/, 8*8 * data.tick/*d*/, 5, 1.); 
   spork ~   SUPERGLIDES2 (RAND.seq("_1,_1,__11,___,1,_1,__11,__,1111",16)/*cutseq*/,8*8 * data.tick/*d*/, 6, 1.); 
