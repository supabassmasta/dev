...
t.scale.size(0);
t.scale << 1 << 3 << 1 << 2 << 3 << 2;
t.reg(SERUM00 s0); s0.config(534);  //data.tick * 8 => t.max; //60::ms => t.glide;  // t.lyd(); // t.ion(); // t.mix();// t.dor();// t.aeo(); // t.phr();// t.loc();
...
STEPC stepc; stepc.init(HW.lpd8.potar[1][6], .2 /* min */, 21 /* max */, 50::ms /* transition_dur */);
