...
// ****  SYNT *****
  ...
  Gain detune[synt_nb];
	TriOsc s[synt_nb];
	Gain final => outlet; .4 => final.gain;

	inlet => detune[i] => s[i] => final;    1. => detune[i].gain;    .6 => s[i].gain; i++; 
	inlet => detune[i] => s[i] => final;    1.01 => detune[i].gain;    .4 => s[i].gain; i++; 
	inlet => detune[i] => s[i] => final;    1.03 => detune[i].gain;    .2 => s[i].gain; i++; 
...
// SYNT
    ...
    .48 => g.gain;
data.tick/8. => f.glide;
   f.adsr.set(2::ms, 15::ms, 0.85,14::ms);
...
//            0-=> pad_nb
                if (pad_nb  == 1) {
                    ...
                    <<<"octave", octave, "offset", note_offset>>>;
										...
										for (0=> int  i; i<s1.s.size(); i++) val $ float / 1024. => s1.s[i].width;
