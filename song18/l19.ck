...
class FROG3 extends SYNT{
  ...
  7 => filter.Q;
   1* 1000 => base.next;
  400 => variable.gain;
  1::second / (data.tick * 18 ) => mod.freq;
  // If mod need to be synced
  // 1 => int sync_mod;
  // if (idx == 0) { if (sync_mod) { 0=> sync_mod; 0.0 => mod.phase; } }
...
*/
TONE t;
t.reg(FROG1 s1);  //data.tick * 8 => t.max; //60::ms => t.glide;  // t.lyd(); // t.ion(); // t.mix();// t.dor();// t.aeo(); // t.phr();// t.loc();
// _ = pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = shift base note , ! = force new note , # = sharp , ^ = bemol  
"*4 ____ 1G__ G/y__ ____" => t.seq;
.9 => t.gain;
