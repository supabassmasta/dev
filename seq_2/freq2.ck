                ...
                if (g.size() != 0) {
                    if (g[int_seq.idx % g.size()] == 0.){
                        ...
                        g[int_seq.idx % g.size()] => adsr.gain;
												new_ev.broadcast();
												if (synt != NULL)
														spork ~ synt.new_note( int_seq.idx);

												0 => int force;
												if ( force_new_note.size() != 0 ) {
													  force_new_note[idx() % force_new_note.size()] => force;
												}
												else {
														0 => force;
												}
                            ...
                            adsr.keyOn();
								...
								// Note offset
								int note_offset_v;
								if (note_offset.size() != 0) {
										note_offset[int_seq.idx % note_offset.size()] => note_offset_v;
								}
								else
										0=>note_offset_v;
...
////                        <<<"target_freq 2:", freq.target()>>>;
//                        <<<"freq.duration 2:", freq.duration()>>>;
                }
