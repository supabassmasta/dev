...
"../_SAMPLES/parrot/parrot1.wav" => s.wav["a"];  // act @=> s.action["a"]; 
// _ = pause , ~ = special pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = rate , ? = proba , $ = autonomous  
"*2_ :2 a__ ____ ____ ____" => s.seq;
.4 * data.potar_synts_gain => s.gain; // s.gain("s", .2); // for single wav 
...
STECHO ech;
ech.connect(last $ ST , data.tick * 3 / 2 , .6);  ech $ ST @=>  last; 
