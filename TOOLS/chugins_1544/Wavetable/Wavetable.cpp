...
void setTable( Chuck_ArrayFloat * ckarray, CK_DL_API api )
{
    ...
    // TOUP: fix double free corruption
    // Only delete if the current table is different of 
    //    CK_SAFE_DELETE_ARRAY( current_table );
    if( current_table != internal_table )
      CK_SAFE_DELETE_ARRAY( current_table );
    else
      current_table = NULL;   // don't free internal_table's buffer here — dtor still owns it
