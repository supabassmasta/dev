...
"../_SAMPLES/bansuri/G/G2.wav" => s.wav["a"];  // act @=> s.action["a"]; 
// _ = pause , ~ = special pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = rate , ? = proba , $ = autonomous  
"a___ ____" => s.seq;
4.9 * data.master_gain => s.gain; // s.gain("s", .2); // for single wav 
...
STGVERB stgverb;
stgverb.connect(last $ ST, .2 /* mix */, 5 * 10. /* room size */, 4::second /* rev time */, 0.2 /* early */ , 0.5 /* tail */ ); stgverb $ ST @=>  last; 
