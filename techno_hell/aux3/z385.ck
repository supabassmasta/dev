...
inlet => detune[i] => s[i] => final;    0.9987 => detune[i].gain;    .3 => s[i].gain; i++;  

fun void config (int n, int m) {
...
TONE t;
t.reg(SERUM1 s0); s0.config(n,m);
t.reg(SERUM1 s1); s1.config(n,m); .7 => s1.outlet.gain;
t.reg(SERUM1 s2); s2.config(n,m); .5 => s1.outlet.gain;
//data.tick * 8 => t.x; //60::ms => t.glide;  // t.lyd(); // t.ion(); // t.mix();//
t.dor();// t.aeo(); // t.phr();// t.loc();
// _ = pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = shift base note , ! = force new note , # = sharp , ^ = bemol  
":8 }c _!2|4|6_5|8|a" => t.seq;
...
STSYNCLPF stsynclpf;
stsynclpf.freq(100 /* Base */, 45 * 100 /* Variable */, 1. /* Q */);
