...
SET_WAV.TRIBAL(s); // SET_WAV.TRANCE_VARIOUS(s);// SET_WAV.TEK_VARIOUS(s);// SET_WAV.TEK_VARIOUS2(s);// SET_WAV2.__SAMPLES_KICKS(s); // SET_WAV2.__SAMPLES_KICKS_1(s); // SET_WAV.BLIPS(s);  // SET_WAV.TRIBAL(s);// "test.wav" => s.wav["a"];  // act @=> s.action["a"]; 
...
s.go();     s $ ST @=> ST @ last; 
.3=> s.wav_o["s"].wav0.rate;
STLPF lpf;
lpf.connect(last $ ST , 3* 1000 /* freq */  , 1.0 /* Q */  );       lpf $ ST @=>  last; 

STADSR stadsr;
stadsr.set(0::ms /* Attack */, 9::ms /* Decay */, .6 /* Sustain */, 5 * 10::ms /* Sustain dur */,  300::ms /* release */);
