...
SEQ s2; SET_WAV.CYMBALS(s2);// s.add_subwav("A", s2.wav["R"]); // s.gain_subwav("K", 0, .3);
 s2.wav["c"]=> s.wav["a"];
//M__M t__S  M_M_ t___    M___ t__S  M_M_ t_M_
//M___ t__M  M_M_ t___    M___ t__S  M__M t_M_
//:4*6ABCABC:6*4 _DDt  :4*6DCBABC:6*4 ADMt
"*4
a|M__M t__S  M_M_ t___
ABCD _DDt  MCBA ADMt

" => s.seq;
.8 * data.master_gain => s.gain; //
s.gain("u", .2); // for single wav 
s.gain("a", .4); // for single wav 
s.gain("A", .3); // for single wav 
s.gain("B", .3); // for single wav 
