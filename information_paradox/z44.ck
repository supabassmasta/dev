...
fun void  SINGLEWAVRATEECHO  (string file, float r, float g){ 
   ...
   STECHO ech;
   ech.connect(last $ ST , data.tick * 4 / 4 , .80);  ech $ ST @=>  last; 

  STAUTOPAN autopan;
  autopan.connect(last $ ST, .4 /* span 0..1 */, data.tick * 2 / 1 /* period */, 0.95 /* phase 0..1 */ );       autopan $ ST @=>  last; 
...
8 *data.tick => w.fixed_end_dur;
  ...
  spork ~   SINGLEWAVRATEECHO("../_SAMPLES/Hawking/supretranslation.wav", 1., 0.7); 
  32 * data.tick =>  w.wait; 

LAUNCHPAD_VIRTUAL.off.set(44);
1::ms => now;
