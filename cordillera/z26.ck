...
t.scale.size(0);
t.scale << 1 << 3 << 1 << 2 << 3 << 2;
t.reg(SERUM00 s0); s0.config(21);  //data.tick * 8 => t.max; //60::ms => t.glide;  // t.lyd(); // t.ion(); // t.mix();// t.dor();// t.aeo(); // t.phr();// t.loc();
// _ = pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = shift base note , ! = force new note , # = sharp , ^ = bemol  
"  
AAAA AA *4 A_A_A_A_ :4
...
stepc.out =>  TriOsc s => Gain gsin=> Gain out;
3 => gsin.op;
STEPC stepc2; stepc2.init(HW.lpd8.potar[1][3], 0 /* min */, 3 /* max */, 50::ms /* transition_dur */);
stepc2.out => gsin;  
