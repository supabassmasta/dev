...
//      15 * 100 => adsrop[i].gain;
      STEPC stepc; stepc.init(HW.lpd8.potar[1][1], 100 /* min */, 9  * 1000 /* max */, 50::ms /* transition_dur */);
      3 => adsrop[i].op;
      stepc.out => adsrop[i];
      i++;

      //---------------------
      opin[i] => SinOsc saw => adsrop[i] => opout[i];
      1./3. +0.1 => opin[i].gain;
      adsrop[i].set(2::ms, 0::ms, 1. , 4::ms);
//      31 => adsrop[i].gain;
      STEPC stepc2; stepc2.init(HW.lpd8.potar[1][2], 0 /* min */, 4  * 1000 /* max */, 50::ms /* transition_dur */);
      3 => adsrop[i].op;
      stepc2.out => adsrop[i];
      i++;
