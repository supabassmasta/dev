...
fun void gen_code (int a, int b){
				"." + conv_hex(a*2)  + "|"  =>  string temp; temp =>s[0].code;  <<<temp>>>;
				"." + conv_hex(b*2)  + "&"  =>   temp; temp =>s[1].code;  <<<temp>>>;
}
...
//            0-=> pad_nb
                if (pad_nb  == 1) {
                    ...
                    <<<"octave", octave, "offset", note_offset>>>;
									  ...
									  1. + (val $ float) / 12700. => s1.detune[1].gain;
                }

								else if (pad_nb  == 7) {
										val => button_0;
										s1.gen_code(button_0, button_1);
                }
