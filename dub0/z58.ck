...
"../_SAMPLES/BluezoneCorporation/GlitchAndDarkMinimalHouse/WAV/Loops 132BPM/Bluezone-GDM-loop-012-132bpm.wav" => buf.read;
.2 => buf.gain;
buf.samples() => buf.pos;
200./132. => buf.rate;
...
s.sync(data.tick/2);// s.element_sync(); //s.no_sync(); //s.full_sync();  // 16 * data.tick => s.extra_end;   //s.print();
