...
///////////////////////////////////////////////////////////////////////////////////////////////
KIK kik;
kik.config(0.1 /* init Sin Phase */, 15 * 100 /* init freq env */, 0.6 /* init gain env */);
kik.addFreqPoint (238.0, 2::ms);
...
fun void SUPERHIGH (string seq) {
  ...
  // _ = pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = shift base note , ! = force new note , # = sharp , ^ = bemol  
  seq => t.seq;
  0.01 * 8 * data.master_gain => t.gain;
  t.no_sync();// t.element_sync();//  t.no_sync();//  t.full_sync(); // 1 * data.tick => t.the_end.fixed_end_dur;  // 16 * data.tick => t.extra_end;   //t.print(); //t.force_off_action();
  // t.mono() => dac;//  t.left() => dac.left; // t.right() => dac.right; // t.raw => dac;
  //t.adsr[0].set(2::ms, 10::ms, .2, 400::ms);
  //t.adsr[0].setCurves(1.0, 1.0, 1.0); // curves: > 1 = Attack concave, other convexe  < 1 Attack convexe others concave
...
//////////////////////////////////////////////////////////////////////////////////
fun void SUPERHIGH3 (string seq) {
  TONE t;
  t.reg(SERUM2 s0);  //data.tick * 8 => t.max; //60::ms => t.glide;  // t.lyd(); // t.ion(); // t.mix();//
  s0.config(3 /* synt nb */ );
  // s0.set_chunk(0); 
  ...
  // _ = pause , | = add note to current , * : = mutiply/divide bpm , <> = groove , +- = gain , () = pan , {} = shift base note , ! = force new note , # = sharp , ^ = bemol  
  seq => t.seq;
  7 * 0.01 * data.master_gain => t.gain;
  t.no_sync();// t.element_sync();//  t.no_sync();//  t.full_sync(); // 1 * data.tick => t.the_end.fixed_end_dur;  // 16 * data.tick => t.extra_end;   //t.print(); //t.force_off_action();
  // t.mono() => dac;//  t.left() => dac.left; // t.right() => dac.right; // t.raw => dac;
  //t.adsr[0].set(2::ms, 10::ms, .2, 400::ms);
  //t.adsr[0].setCurves(1.0, 1.0, 1.0); // curves: > 1 = Attack concave, other convexe  < 1 Attack convexe others concave
...
STFILTERX stlpfx0; LPF_XFACTORY stlpfx0_fact;
stlpfx0.connect(last $ ST ,  stlpfx0_fact, 15* 100.0 /* freq */ , 1.0 /* Q */ , 3 /* order */, 1 /* channels */ );       stlpfx0 $ ST @=>  last;  
...
STECHO ech2;
ech2.connect(last $ ST , data.tick * 3 / 4 , .93);  ech2 $ ST @=>  last; 

STAUTOPAN autopan2;
autopan2.connect(last $ ST, .5 /* span 0..1 */, data.tick * 3 / 4 /* period */, 0.25 /* phase 0..1 */ );       autopan2 $ ST @=>  last; 
...
//  spork ~  BASS0 ("*4   __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1    __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1 __!1!1_    "); 
  spork ~   PLOC ("____  ____ __ " + RAND.char("1853B__",5)); 
  spork ~ SUPERHIGH3 (" __ " + RAND.char("1853B__",4)); 
  ...
  spork ~   NIAP (" _1|3|5__ ____", 8 /*synt_nb  */, 200::ms /* decay_dur */, 2.0 /* decay_curve */,  .36 /* gain */);
  spork ~   PLOC ("____  ____ __ " + RAND.char("1853B__",5)); 
  spork ~ SUPERHIGH2 ("{c ____ ____ ____ __ " + RAND.char("1853B__",3)); 

  spork ~ TRIBAL("____ __*4    "+ RAND.seq("xx__,a_a_,HH__,I___,LLL_,{4LLL_,{4L}6LL_,{4M}4M__,}4M}4MM_",1) +":4 _ ____ __   "+ RAND.seq("xx__,a_a_,HH__,I___,LLL_,{4LLL_,{4L}6LL_,{4M}4M__,}4M}4MM_",1), 0 /* bank */, 1 /* tomix */, 0.44 /* gain */);
 
  16 * data.tick =>  w.wait; 
  spork ~   PLOC ("____  ____ __ " + RAND.char("1853B__",5)); 
  spork ~ SUPERHIGH ("____ ____ ____ __ " + RAND.char("1853B__",3)); 
...
fun void  LOOPLAB2  (){ 
  while(1) {
  spork ~ TRIBAL("*4 hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh hhhh ", 2 /* bank */, 0 /* tomix */, 0.44 /* gain */);
//  spork ~ TRIBAL("*4 h_hj h_hh h_j_ hhh_ h_hj h_h_ h_jj h_h_ h_hj h_hh h_jh h_h_ h_hj h_h_ h_jj h_hh  ", 2 /* bank */, 0 /* tomix */, 0.44 /* gain */);
  spork ~   NIAP (" _1|3|5__ ____", 8 /*synt_nb  */, 200::ms /* decay_dur */, 2.0 /* decay_curve */,  .36 /* gain */);
//  spork ~ TRIBAL("*4 __h_ __j_h_h_ __j___h_ h_j___h_ __j__hh_ __j___h_ __j_h_h_ h_j___h_ h_jh ", 2 /* bank */, 0 /* tomix */, 0.44 /* gain */);
//  spork ~ TRIBAL("*4 _h_j _h_j _h_j _h_j_h_j _h_j_h_j _h_j_h_j _h_j_h_j _h_j_h_j _h_j_h_j _h_j", 2 /* bank */, 0 /* tomix */, 0.44 /* gain */);
...
fun void  LOOPLAB3  (){ 
  ...
  spork ~  BASS0 ("*4   __!1!1 __!1!1 __!1!1 __!1!1  __!1!1 __!1!1 __!1!1 __!1!1    __!4!4 __!4!4 __!4!4 __!4!4  __!5!5 __!3!3 __!4!4 __!5!5_    "); 
  spork ~ TRANCEHH ("*2 _hsh_hsh _hsh_hsh  _hsh_hsh _hsh_s_T"); 
  16 * data.tick =>  w.wait; 
  }
} 
