class synt0 extends SYNT{
    ...
    adsrop[i].set(.5*data.tick, .5*data.tick, .00001 , 200::ms);
    ...
    1./4. +0.000 => opin[i].gain;
    adsrop[i].set(2::ms, 186::ms, 1 , 400::ms);
    adsrop[i].setCurves(1.0, 1.0, 1.0); // curves: > 1 = Attack concave, other convexe  < 1 Attack convexe others concave
    30 => adsrop[i].gain;

    fun void f1 (){ 
while(1) {
    LPD8.pot[1][1] * 100 => adsrop[i].gain;
...
STFILTERMOD fmod;
fmod.connect( ech , "LPF" /* "HPF" "BPF" BRF" "ResonZ" */, 3 /* Q */, 600 /* f_base */ , 400  /* f_var */, 2::second/data.tick /* f_mod */); 

STREV2 rev; // DUCKED
rev.connect(fmod $ ST, .1 /* mix */); 
